...
function arlo_tm_contact_form(){
	...
	jQuery(".contact_form #send_message").on('click', function(){
		...
		if(name===''||email===''||message===''){
			...
			var url = "https://script.google.com/macros/s/AKfycbytQnhPUUmv3eZelPMTETcXBXWnehlZaL6-7mvF6HTYs6Gx5yw/exec";
			...
			$.ajax({
	            ...
	            success: function (data)
	            {
					...
					if(data.result=="success"){
						jQuery(".contact_form .returnmessage").append("<span class='contact_success'>Yor message has been sent.</span>");
...
function arlo_tm_magnific_popup(){
	...
	jQuery('.open-popup-portfolio').magnificPopup({
		type:'image',
		titleSrc: 'title',
...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["Android App Developer", "Web Developer", "Programmer", "UI/UX Designer"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
