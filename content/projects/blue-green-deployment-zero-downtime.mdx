...
<Accordion>
    <AccordionItem title="Why use `set -euo pipefail` at the top of the script?">
        ...
        non-zero command, so a failed `cp` cannot lead to a half-rewritten config.
        `-u` aborts when an undefined variable is read, which catches typos
        in environment names. `-o pipefail` propagates failures from any stage
        of a pipeline, not just the last one. Together they make the script fail
        loudly and early instead of leaving production in a broken state.
    </AccordionItem>
    <AccordionItem title="Why drain for thirty seconds instead of stopping the old container immediately?">
        A user who is mid-request when the flip happens is still connected to the
        old container's worker. Killing that container immediately drops the
        response. The drain window is a deliberate buffer that lets in-flight
        requests finish on the old workers before the container is stopped.
        Tune the value to the slowest response your application produces.
    </AccordionItem>
    ...
    <AccordionItem title="Where should the deploy script run from?">
        ...
        directly on the host as a privileged user (it needs to call
        `nginx -s reload`). For teams that prefer not to give CI servers SSH
        access, wrap the script in a small webhook receiver on the host that
        triggers it on a push to `main`.
    </AccordionItem>
</Accordion>
